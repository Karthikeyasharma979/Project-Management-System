...
const SignUp = ({ setSignUpOpen, setSignInOpen }) => {
  ...
  const createAccount = () => {
    if (otpVerified) {
      ...
      setLoading(true);
      try {
        signUp({ name, email, password, gender }).then((res) => {
          if (res.status === 200) {
  ...
  const handleSignUp = async (e) => {
    ...
    if (name === "" || email === "" || password === "") {
      ...
      dispatch(
        openSnackbar({
          message: credentialError || emailError || "Please fill all the fields correctly",
          severity: "error",
        })
      );
    }
  ...
  return (
    <Modal open={true} onClose={() => setSignUpOpen(false)}>
      <Container>
        <Wrapper>
          <CloseRounded
            style={{
